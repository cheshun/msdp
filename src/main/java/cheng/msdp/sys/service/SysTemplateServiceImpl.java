...
public class SysTemplateServiceImpl implements ISysTemplateService {
	...
	List<DataDictItemModel> list = (List<DataDictItemModel>) dataBaseService
			.queryByClause(DataDictItemModel.class, "pk_datadict='" + pk_datadict + "' and isBilltemplatesyn= '" + UFBoolean.FALSE + "'");
	...
	DataDictModel datadict = (DataDictModel) dataBaseService.queryByPK(DataDictModel.class, pk_datadict);
	List<DataDictItemModel> list =  dataBaseService
			.queryByClause(DataDictItemModel.class, "pk_datadict='" + pk_datadict + "' and isQueryTemplatesyn= '" + UFBoolean.FALSE + "'");
    ...
    public boolean createBillTemplate(DataDictModel datadict, List<DataDictItemModel> list) throws BusinessException {
	//生成单据模板主表信息
	...
	if (node.getFun_type().equals("report")) {
	    List<ReportTemplateModel> list =   dataBaseService.queryByClause(ReportTemplateModel.class, " pk_node='" + datadict.getPk_node() + "' and dr=0 ");
	    ...
	    List<UITempletModel> list =  dataBaseService.queryByClause(UITempletModel.class, " pk_node='" + datadict.getPk_node() + "' and dr=0 ");
	    if (list == null || list.size() == 0) {
		//如果没有则添加一个默认的模板
		//如果有 返回所有的模板主键
     ...
     * @throws BusinessException
     */
    private void createBillItemTemplate(DataDictModel datadict, String[] pk_billtemplate, List<DataDictItemModel> list) throws BusinessException {
	NodeModel node = (NodeModel) dataBaseService.queryByPK(NodeModel.class, datadict.getPk_node());
	if (node.getFun_type().equals("report")) {
     ...
     * 生成查询模板信息
     */
    public boolean createQueryTemplate(DataDictModel datadict, List<DataDictItemModel> list) throws BusinessException {
	//生成查询模板主表信息
     ...
     * @throws BusinessException
     */
    private void createQueryItemTemplate(String[] pk_querytemplate, List<DataDictItemModel> list) throws BusinessException {
